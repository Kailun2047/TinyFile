...
/****************************************/
extern void call_service(char* filename, size_t file_id, char* result) {
	...
	int shmids[chunk_tol]; // also store the shared memory ids to destroy them later
	...
	size_t received_cnt = 0, cursor_pos = 0;
	while (received_cnt < chunk_tot) {
		...
		if (msgrcv(ipc_shared->msgq_id, msgp, sizeof(char)*SIZE_T_BYTES*4, file_id + 2, MSG_NOERROR | IPC_NOWAIT) != -1) {
			...
			memcpy(result + cursor_pos, buffers[cur_chunk], read_bytes);
			shmdt(buffers[cur_chunk]);
			shmctl(shmids[cur_chunk], IPC_RMID, NULL);

			cursor_pos += read_bytes;
		}
	}
}